...
def test_apprun_and_build_scripts_are_executable() -> None:
    paths = (
        ...
        PROJECT_ROOT / "scripts" / "build-bundled-ffmpeg.sh",
...
def test_custom_qt_hook_excludes_broken_tiff_plugin() -> None:
    hook = (PROJECT_ROOT / "packaging" / "pyinstaller-hooks" / "hook-PySide6.QtGui.py").read_text(
        encoding="utf-8"
    )
...
def test_build_requirements_are_pinned() -> None:
    requirements = (PROJECT_ROOT / "requirements-build-linux.txt").read_text(encoding="utf-8")
...
def test_build_script_packages_license_and_build_information() -> None:
    build_script = (PROJECT_ROOT / "scripts" / "build-appimage.sh").read_text(encoding="utf-8")

    assert "THIRD_PARTY_NOTICES.md" in build_script
    assert '"$PROJECT_ROOT/NOTICE"' in build_script
    assert "FFMPEG-SOURCE.md" in build_script
    ...
    assert "FFMPEG-LGPL-2.1.txt" in build_script
...
def test_build_script_pins_appimagetool_and_creates_portable_checksum() -> None:
    ...
    assert "PYSIDE_QT_LIBRARY_DIR" in build_script
    assert 'env LD_LIBRARY_PATH="$PYINSTALLER_LIBRARY_PATH"' in build_script
    assert "--mksquashfs-opt=-processors" in build_script
    assert "--mksquashfs-opt=-all-time" in build_script


def test_build_script_verifies_ffmpeg_and_corresponding_source() -> None:
    build_script = (PROJECT_ROOT / "scripts" / "build-appimage.sh").read_text(encoding="utf-8")
    hashes = (PROJECT_ROOT / "packaging" / "linux" / "ffmpeg-x86_64.sha256").read_text(
        encoding="utf-8"
    )
    source_hash = (PROJECT_ROOT / "packaging" / "linux" / "ffmpeg-source-7.0.2.sha256").read_text(
        encoding="utf-8"
    )
    ...
    assert len(source_hash.split()[0]) == 64
    assert "FFmpeg source archive SHA-256" in build_script
    assert "FFMPEG_SOURCE_BUNDLE" in build_script
    assert "FFMPEG_RUNTIME_BUNDLE" in build_script
    assert "Corresponding FFmpeg source release is missing" in build_script
    assert "Reviewed FFmpeg runtime release is missing" in build_script
    assert "--disable-gpl" in build_script
    assert "--disable-nonfree" in build_script

    appdir_test = (PROJECT_ROOT / "scripts" / "test-appdir.sh").read_text(encoding="utf-8")
    assert '"$APPDIR/usr/bin/ffmpeg" -version' in appdir_test
    assert '"$APPDIR/usr/bin/ffprobe" -version' in appdir_test


def test_bundled_ffmpeg_builder_uses_pinned_official_lgpl_source() -> None:
    script = (PROJECT_ROOT / "scripts" / "build-bundled-ffmpeg.sh").read_text(encoding="utf-8")

    assert "https://ffmpeg.org/releases/$SOURCE_NAME" in script
    assert "ffmpeg-source-7.0.2.sha256" in script
    assert "ffmpeg-runtime-7.0.2.sha256" in script
    assert "--disable-autodetect" in script
    assert "--disable-gpl" in script
    assert "--disable-nonfree" in script
    assert "GNU Lesser General Public" in script
    assert "COPYING.LGPLv2.1" in script
    assert "source-bundle.tar.xz" in script
    assert "runtime-bundle.tar.xz" in script


def test_windows_spec_never_autodetects_ffmpeg() -> None:
    spec = (PROJECT_ROOT / "toolbox_lightweight.spec").read_text(encoding="utf-8")

    assert "TOOLBOX_FFMPEG_BINARY" in spec
    assert "TOOLBOX_FFPROBE_BINARY" in spec
    assert "shutil.which" not in spec
    assert "project_root / binary_name" not in spec
    assert "third_party" not in spec


def test_appimage_acceptance_covers_content_relocation_and_xcb() -> None:
    build_script = (PROJECT_ROOT / "scripts" / "build-appimage.sh").read_text(encoding="utf-8")
    image_test = (PROJECT_ROOT / "scripts" / "test-appimage.sh").read_text(encoding="utf-8")
    content_test = (PROJECT_ROOT / "scripts" / "check-appimage-content.sh").read_text(
        encoding="utf-8"
    )
    elf_test = (PROJECT_ROOT / "scripts" / "check-elf-dependencies.sh").read_text(
        encoding="utf-8"
    )
    ...
    assert 'BUNDLE_LIBRARY_PATH="$TARGET/_internal"' in elf_test
    assert 'LD_LIBRARY_PATH="$BUNDLE_LIBRARY_PATH" ldd' in elf_test


def test_reproducibility_check_compares_content_modes_and_symlinks() -> None:
    comparison_script = (PROJECT_ROOT / "scripts" / "compare-appimage-contents.sh").read_text(
        encoding="utf-8"
    )
...
def test_x11_acceptance_checks_window_identity_resize_and_xdg_state() -> None:
    x11_test = (PROJECT_ROOT / "scripts" / "test-x11-desktop.sh").read_text(encoding="utf-8")
